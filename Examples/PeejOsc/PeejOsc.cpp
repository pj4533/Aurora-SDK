...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size)
{
	...
	hw.SetLed(LED_1, 0.f, 0.f, chord_size >= 1.0);
	hw.SetLed(LED_2, 0.f, 0.f, chord_size >= 2.0);
	hw.SetLed(LED_3, 0.f, 0.f, chord_size >= 3.0);
	hw.SetLed(LED_4, 0.f, 0.f, chord_size >= 4.0);
	hw.SetLed(LED_5, 0.f, 0.f, chord_size >= 5.0);
	hw.WriteLeds();
 
	// Could probably tighten this code up, rather than if/thens here. Left it
	// verbose so it would be easier to understand.
